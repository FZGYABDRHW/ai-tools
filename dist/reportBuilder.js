...
const taskIterator_1 = require("./taskIterator");
 ...
 */
async function buildReport(rawPrompt, si, taskPlotGenerator, onProgress, onParametersExtracted, abortSignal, startOffset = 0, parameters, existingColumns, onStatusUpdate) {
    ...
    const openai = new openai_1.default({ dangerouslyAllowBrowser: true, apiKey });
...
Calculate dates dynamically using the current date provided above. Return only the JSON object, no additional text.`
    ...
    const results = [];
    const makeTaskPrompt = (taskText) => `Context: ${rawPrompt}\n` +
    ...
    await new Promise((resolve, reject) => {
        ...
        const subscription = (0, taskIterator_1.taskIterator)(si, startOffset, extractedParameters).pipe(
        // We only care about concrete "content" events (those carry the task ID).
        (0, operators_1.filter)((ev) => ev.type === 'content'), 
        // Add abort check to the stream
        (0, operators_1.filter)(() => !abortSignal?.aborted), 
        ...
        (0, operators_1.mergeMap)(async (ev) => {
            ...
            const hasEmptyRequiredFields = parsed
                ? columns.some((col) => col !== 'taskId' && isValueEmpty(parsed[col]))
                : false;
