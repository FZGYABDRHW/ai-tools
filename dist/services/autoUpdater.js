...
class AutoUpdaterService {
    ...
    setupAutoUpdater() {
        ...
        console.log("Current app version:", electron_1.app.getVersion());
        console.log("App path:", electron_1.app.getAppPath());
        console.log("App name:", electron_1.app.getName());
        // Check if we're in development or production
        const isDev = process.env.NODE_ENV === "development" || !electron_1.app.isPackaged;
        console.log("Is development:", isDev);
        console.log("Is packaged:", electron_1.app.isPackaged);
        // Log the expected app-update.yml path
        // In development, use the project root directory instead of parent of app path
        const expectedUpdatePath = isDev
            ? path.join(process.cwd(), "app-update.yml")
            : path.join(electron_1.app.getAppPath(), "..", "app-update.yml");
        ...
        try {
            if (fs.existsSync(expectedUpdatePath)) {
                ...
                // Skip in development if we can't write to the file system
                if (isDev) {
                    console.log("⚠️  Skipping app-update.yml creation in development mode");
                }
                else {
...
`;
            ...
            console.log("⚠️  Continuing without app-update.yml file");
        }
    }
    ...
    setupIpcHandlers() {
        electron_1.ipcMain.handle("auto-updater:check-for-updates", async () => {
            try {
        ...
        electron_1.ipcMain.handle("auto-updater:download-update", async () => {
            try {
                console.log("Download update requested");
                const result = await electron_updater_1.autoUpdater.downloadUpdate();
        ...
        electron_1.ipcMain.handle("auto-updater:install-update", () => {
            console.log("Install update requested");
            electron_updater_1.autoUpdater.quitAndInstall();
        });
        electron_1.ipcMain.handle("auto-updater:get-app-version", () => {
            return electron_1.app.getVersion();
        });
        electron_1.ipcMain.handle("auto-updater:download-to-disk", async (event, updateInfo) => {
            try {
        ...
        electron_1.ipcMain.handle("auto-updater:force-check-for-updates", async () => {
            try {
        ...
        electron_1.ipcMain.handle("auto-updater:test-ipc", () => {
        ...
        electron_1.ipcMain.handle("auto-updater:download-latest-version", async () => {
            try {
    ...
    async downloadUpdateToDisk(updateInfo) {
    ...
    async cleanupDownloadedFile(version) {
        try {
            const downloadsPath = electron_1.app.getPath("downloads");
