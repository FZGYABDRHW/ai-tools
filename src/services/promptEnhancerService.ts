...
export interface PromptEnhancementResult {
    ...
    suggestions?: string[]; // legacy hints
    aiSuggestions: AISuggestionField[];
}
...
export class PromptEnhancerService {
    ...
    private initializeOpenAI(): void {
        ...
        this.openai = new OpenAI({ dangerouslyAllowBrowser: true, apiKey });
    }
...
Reflect ALL conditions as either fields or allowed filters (Status, Date Range, Limit).`
    ...
    private async generateAISuggestions(originalPrompt: string, enhancedMarkdown: string): Promise<AISuggestionField[]> {
        try {
            const completion = await this.openai!.chat.completions.create({
                model: 'gpt-4o',
                messages: [
...
- Include data type, format, and possible values in descriptions when relevant:
  ...
  * Enums: list possible values (e.g., "Возможные значения: новый, в работе, завершен")`
            ...
            try {
                const parsed = JSON.parse(raw) as AISuggestionField[];
    ...
    async getAISuggestions(originalPrompt: string, enhancedMarkdown: string): Promise<AISuggestionField[]> {
        if (!this.openai) {
            this.initializeOpenAI();
        }
    ...
    async mergeWithSuggestions(enhancedMarkdown: string, suggestions: AISuggestionField[], originalPrompt: string): Promise<string> {
        if (!this.openai) {
            this.initializeOpenAI();
        }
...
- Keep formatting consistent (bold field names, concise descriptions). Return ONLY the final Markdown.`
    ...
    static shouldEnhance(prompt: string): boolean {
        ...
        const hasVagueTerms = vagueTerms.some(term =>
            prompt.toLowerCase().includes(term) && prompt.length < 100
        );
