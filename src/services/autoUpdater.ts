...
export class AutoUpdaterService {
  ...
  private setupAutoUpdater() {
    ...
    console.log(
      "Setting up Smart Automatic auto-updater for public repository"
    );
    ...
    try {
      if (fs.existsSync(expectedUpdatePath)) {
        ...
        if (isDev) {
          console.log(
            "⚠️  Skipping app-update.yml creation in development mode"
          );
        } else {
    ...
    autoUpdater.on("update-downloaded", (info) => {
      ...
      this.sendStatusToWindow(
        "update-downloaded",
        "Update ready! Click to install or it will install on quit.",
        serializableInfo
      );
...
`;
  ...
  private showInstallDialog(info: any) {
    if (this.mainWindow) {
      this.mainWindow.webContents.send(
        "auto-updater:show-install-dialog",
        info
      );
    }
  }
  ...
  async checkForUpdates() {
    try {
      ...
      const serializableResult = this.makeSerializable(result);
      console.log(
        "Public checkForUpdates serializable result:",
        serializableResult
      );
      return serializableResult;
    } catch (error) {
  ...
  async downloadUpdate() {
    try {
      ...
      const serializableResult = this.makeSerializable(result);
      console.log(
        "Public downloadUpdate serializable result:",
        serializableResult
      );
      return serializableResult;
    } catch (error) {
  ...
  async downloadLatestVersion() {
    try {
      ...
      const response = await fetch(
        "https://api.github.com/repos/FZGYABDRHW/ai-tools/releases/latest"
      );
      if (!response.ok) {
  ...
  private async validateDownloadsAccess(): Promise<void> {
    try {
      ...
      throw new Error(
        "Cannot write to Downloads folder. Please check permissions."
      );
    }
  }
  ...
  async downloadUpdateToDisk(updateInfo: any): Promise<any> {
    ...
    try {
        ...
        if (value) {
          ...
          this.sendStatusToWindow(
            ...
            `Downloading ${fileName}...`,
            {
              ...
              total: contentLength,
              bytesPerSecond:
                downloadedBytes / ((Date.now() - downloadStartTime) / 1000),
          ...
          console.log(
            `📥 Progress: ${Math.round(percent)}% (${this.formatFileSize(downloadedBytes)}/${this.formatFileSize(contentLength)})`
          );
        }
      }
      ...
      console.log(
        "   - Download time:",
        ((Date.now() - downloadStartTime) / 1000).toFixed(2),
        "seconds"
      );
      ...
      if (contentLength > 0 && stats.size !== contentLength) {
        throw new Error(
          `File size mismatch: expected ${this.formatFileSize(contentLength)}, got ${this.formatFileSize(stats.size)}`
        );
      }
      ...
      this.sendStatusToWindow(
        ...
        `Download complete: ${fileName}`,
        {
          ...
          downloadTime:
            ((Date.now() - downloadStartTime) / 1000).toFixed(2) + "s",
        }
      );
